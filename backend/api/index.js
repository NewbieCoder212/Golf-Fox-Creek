...
async function updateProfileAfterInvite(params) {
  ...
  const existing = await adminFetch(`/rest/v1/user_profiles?id=eq.${params.userId}&select=id&limit=1`);
  if (existing.ok && existing.data?.[0]) {
    const { ok: ok2, data: data2 } = await adminFetch(`/rest/v1/user_profiles?id=eq.${params.userId}`, {
      method: "PATCH",
      body
    });
    if (!ok2) {
      throw new Error(getErrorMessage(data2));
    }
    return;
  }
...
// src/lib/tournament-participant-onboarding.ts
...
async function loadParticipantOnboarding(tournamentId) {
  ...
  const [playersResult, authByEmail] = await Promise.all([
    supabase.from("tournament_players").select("id,email,user_id,invite_email_sent_at").eq("tournament_id", tournamentId).order("display_name"),
...
async function ensureUserProfileForInvite(params) {
  const supabase = getSupabaseAdminClient();
  const { data: existing } = await supabase.from("user_profiles").select("id").eq("id", params.userId).limit(1).maybeSingle();
  if (existing?.id) {
    return;
  }
  const { error } = await supabase.from("user_profiles").insert({
    ...
    first_name: params.firstName.trim(),
    last_name: params.lastName.trim(),
    full_name: buildFullName2(params.firstName, params.lastName),
    role: "member",
...
tournamentTeamsRouter.get("/:tournamentId/participant-onboarding", requireManagerAuth, async (c) => {
  const tournamentId = c.req.param("tournamentId");
  try {
