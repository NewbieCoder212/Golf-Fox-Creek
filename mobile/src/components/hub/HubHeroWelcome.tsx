...
import { Globe, LogOut, Target, Star } from 'lucide-react-native';
...
function getHeroHeight(containerWidth: number, windowHeight: number): number {
  ...
  const heightBased = Math.round(windowHeight * 0.44);
      ...
      <View className="flex-1 px-5 pt-3 pb-5 justify-between">
        ...
        <View className="flex-row gap-2 mt-4">
          <Pressable
            ...
            className="flex-1 flex-row items-center bg-black/50 rounded-xl px-3 py-2.5 border border-white/10 active:opacity-80"
          >
            <Target size={16} color={foxColors.lime} strokeWidth={1.5} />
            <View className="ml-2 flex-1">
              <Text className="text-neutral-300 text-[10px] font-body-semibold uppercase tracking-wide">
                {t.handicap}
              </Text>
              {loading ? (
          ...
          <View className="flex-1 flex-row items-center bg-black/50 rounded-xl px-3 py-2.5 border border-white/10">
